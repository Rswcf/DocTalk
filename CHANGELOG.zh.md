...
### 新增
- 免费账户现在可以试用一次 Domain Mode。这个法律/学术对话模式是用户点击
  "升级"最主要的原因,而在此之前没有人真正见过自己被要求付费的功能。

### 变更
- 升级按钮现在直接打开 Stripe。此前应用内所有升级入口都只是把你送到账单页,
  真正的订阅按钮还要自己往下翻着找;现在表达意愿的那一次点击就开始结账。
- Plus 上传上限提升到 100 MB(此前 50 MB,与免费版相同),Pro 提升到 200 MB
  (此前 100 MB)。文件大小是用户最常撞上的限制,而 Plus 此前并没有真的放宽它。
- 文档新增页数上限:免费 750 页、Plus 1,500 页、Pro 3,000 页。现有文档全部
  在限额之内。
- 从网址导入统一为 10 MB 上限,所有套餐一致。
- 演示页直接展示示例文档,不再被标题、副标题和三步说明挡在下面。

### 修复
- 结账失败现在会告诉你到底哪里出了问题,而不是一句笼统的错误。
- "试试 Quote Finder"提示现在会在你用日常说法提问时出现——"在哪页写的"、
  "出处是什么"、"原文在哪"——而不再只认少见的学术措辞。它依然不会自行发起检索。
- 加密的 PDF 与无法确定长度的文件现在在上传时就会被明确拒绝并说明原因,
  不再先存下来、之后再失败。

### 内部
- 2026-08-26 自上而下复盘的增长批次 A 与 C。共十轮对抗审查(A 七轮、C 三轮)
  至共识。过程中发现并修复了后台文档任务积分结算中四个既有缺陷,其中之一
  会因为对已结算的工作重复退款而凭空产生积分。记录见
  `.collab/reviews/2026-08-27-batch-{a,c}-CONSENSUS.md`。

## [0.28.1] - 2026-08-08

### 修复
- 文档解析恢复正常速度。自 5 月底起,后端与数据库被平台调度到了不同大洲,
  每次数据库往返都要付出跨洋延迟,而解析又是逐行写入数千行数据——上传要等
  数分钟,超大文档甚至直接失败。现已把后端迁回数据层同区,并将页面、结构
  元素与切块改为批量写入。
- 解析超时的文档现在会如实报告为超时(并自动重试),不再被误标为数据库
  故障后放弃。

## [0.28.0] - 2026-08-04

### 变更
- 营销站全新视觉:暖纸底上的苹果风磨砂玻璃 chrome,配色更新(深锈红=行动、
  普鲁士蓝=结构、橄榄绿专属已验证状态),字体升级(Fraunces 标题衬线 +
  IBM Plex Sans)。正文保持实心表面——玻璃只用于导航、卡片与控件;所有玻璃
  表面在"减少透明度"与打印场景下回退为实心。
- 公开分享页并入营销设计体系,引文文本达到 AA 对比度。

### 修复
- 营销页头恢复吸顶(自 5 月起失效)——内容从磨砂玻璃下滚过。
- 暖纸背景现在真正渲染在每个营销页上;此前除落地页外均被静默压制。
- 分享的对话恢复段落换行。

### 内部
- Liquid Glass 计划第 1 批(营销/editorial 层)。16 提交,逐任务对抗审查;
  上线前发现并修复 7 个缺陷,含两个潜伏的生产 bug。记录见
  `.superpowers/sdd/2026-08-04-liquid-glass-*` 与
  `.collab/reviews/2026-08-04-glass-b1-*`。app 层为第 2 批。

## [0.27.0] - 2026-08-04

### 修复
- Quote Finder 现在会交出那些它本来找到、却被丢弃的引文。当一段话跨越翻页时,
  DocTalk 会以诚实的双页出处("pp. 30–31")展示它,而不是直接丢掉。用我们自己
  用户文档上的真实搜索重放,验证通过的引文从 10 条里的 1 条升到 8 条——其中
  包含此前被丢失的、一字不差的精确匹配。
- 复制跨页引文时,引注会正确写成 "pp. 30–31",不再悄悄只标首页。

### 内部
- 恰好跨一个页边界的 `extracted_text` 片段发出诚实区间并附两页 bbox;跨两个及
  以上边界仍按 `ambiguous_page_range` 丢弃;bbox 多数票猜页依然禁止。这是对 M2
  归因策略的一次有据修正案,依据是留存学术语料的生产内重放
  (`.collab/reviews/2026-08-04-m3-acceptance-gate.md`),并通过三轮 Codex 对抗审查。

## [0.26.0] - 2026-08-04

### 修复
- 法律模式与学术模式现已正确限定为 Plus 和 Pro 套餐。此前它们在界面上标为
  付费功能,但服务端对任何人都放行,免费账号直接请求即可获得付费行为。现在
  接受该参数的两处入口(聊天与结构化提取)都会校验套餐。
- 触达套餐上限不再是死胡同。分享链接用尽、版式翻译时文档额度已满、请求了
  未订阅的领域模式,现在都会给出明确的下一步,而不是一条干巴巴的报错。
- 已经是 Pro 却触达文档上限时,DocTalk 会提示你清理空间,而不再给出
  "降级到 Plus"的链接。

### 内部
- domain_mode 在两处请求入口均以 403 DOMAIN_MODE_REQUIRES_PLUS 门控;会话
  持久值在每条成功路径上同步为当前请求的值,且并入该路径自身的事务(无独立
  提交、无半提交元数据、无未退款窗口)。经两轮 Codex 对抗审查与真实 Postgres
  失败注入覆盖;记录见 `.collab/reviews/2026-08-0{3,4}-p1-*`。

## [0.25.0] - 2026-08-03

### 新增
- **证据看板 Evidence Board**:任意已验证引文卡一键保存,在 Quote Finder 的
  "已保存"标签页中随时找回——含可就地编辑的笔记、页码、信任标签、跳转高亮与
  带引注复制。个人资料页新增"已保存引文"标签,汇总你所有文档的保存引文。
  免费版含 30 条有效保存额度(删除即释放);重复保存同一条不重复计数。
- 保存时服务端重新验证——保存的卡片只可能包含经服务端对照你的文档验证过的
  文本,且每条都记录了验证依据以备将来复核。
- 账户数据导出现已包含你保存的引文与笔记。

### 内部
- 保存时验证锚(来源哈希+偏移);按用户的顾问锁封堵并发越限;为重解析引用
  动作补部分索引;保存引文全生命周期的真实 ASGI+Postgres 集成覆盖。四轮
  Codex 对抗审查(REVISE → r4 共识通过);记录见 `.collab/reviews/2026-08-03-m3-*`。

## [0.24.0] - 2026-08-02

### 新增
- **引文查找器 Quote Finder**(登录用户):在任意文档输入主题,获得经机器逐字
  验证的引文卡片——精确原文、页码、一键跳转到高亮段落、带引注复制
  ("作者, 年份, p. X")。未经服务端对照存储文本验证的引文绝不展示——结果会
  如实标注是对照原始页面文本还是提取文本验证的。
- 每文档可编辑的引注元数据(作者/年份/标题),自动从文件预填,始终可修改。
- 在聊天中请求"逐字引用/原文引用"会路由到验证式引文管线;含糊的表述则显示
  非侵入式"试试 Quote Finder"提示而不是猜测。引文搜索按 Pro 聊天消息计费;
  未验证出结果的搜索只收实际成本。
- 新上传的 PDF 现在按页存储原始文本,支撑页面级验证与页码精确归因。

### 修复
- Demo 示例文档启动时自愈其存储的 PDF 文件——此前一次存储迁移悄悄丢失了
  demo PDF(聊天正常但 PDF 面板打不开);已恢复,且此类问题不会再无声复发。
- 积分对账引入结算标记且并发安全:无论聊天还是引文搜索路径,扣费不会再被
  双重退款、结算后退款,或因请求取消而悬空。

### 内部
- 68 个提交,三波子代理团队开发,内部逐波审查 + 六轮 Codex 对抗审查
  (r1 BLOCK → r6 共识通过);记录见 `.collab/reviews/2026-08-02-quote-finder-m2-*`。
  集成测试改用隔离的临时数据库,仅允许本机回环提供。

## [0.23.0] - 2026-08-02

### 修复
- Demo 现在真正兑现宣传的额度:每个示例文档 5 条免费消息(此前是三个示例文档
  共享一个计数器),且计数在刷新页面、切换语言、网络波动后都保持准确——会自动
  与服务端计数重新同步。
- 在示例文档之间来回浏览不再悄悄消耗会话配额——回到某个示例文档时会恢复之前
  的对话而不是重新开始,正常浏览也不会再触发"请稍候"的限流提示。
- 示例文档不会再永久失效:会话上限改为 24 小时滚动窗口(此前是终身累计),每晚
  自动清理被遗弃的空会话,已登录账号也无法再耗尽匿名 Demo 的容量。
- 恢复了浅色模式下约 40 处因早前主题改造而"隐形"的界面元素:输入中动画、流式
  光标、输入框提示文字、Beta 徽章、点赞高亮、各类悬停状态等。深色模式不变。
- Demo 页现已提供全部 11 种语言的本地化网址(如 /de/demo),首屏即为翻译后的
  内容,对搜索引擎完全可见。
- 达到 Demo 上限后点击"上传你自己的文档",登录后会进入上传台而不是回到 Demo。
- Demo 中现在可以看到分享按钮,未注册访客会看到诚实的提示文案"登录后即可分享
  此对话"。

### 内部
- 匿名 Demo 消息计量按(IP,文档)维度;会话上限只统计 24 小时窗口内的匿名会话
  并配套部分索引;登录确认遥测(auth_confirm_*)现接受未登录客户端上报。
- 本批次由三波子代理团队开发,经 6 轮 Codex 对抗审查(r1 BLOCK → r6 共识通过)
  打磨;审查记录见 `.collab/reviews/2026-08-02-p0-*`。

## [0.22.0] - 2026-07-04

### 修复
- 邮箱登录链接现在会先打开确认页,点击按钮后才完成登录。此前企业邮箱的安全扫描
  程序会在收件人点击之前静默消耗一次性登录链接——造成死链和幽灵账号。如果你用
  工作邮箱登录时曾遇到 "Configuration" 错误,此问题已修复。
- 精确引用增强不再对匿名 demo 会话运行;增加了时间预算保护,长回答不会因此超过
  代理时限;运行时显示"Refining citations…"状态;其模型用量现已正确入账。
- 打开没有生成建议问题的文档时,不再显示上一个文档的建议问题。
- 更正了功能页、一篇博客与内部文档中过期的免费套餐文案(300 积分、50 MB 上传)。

### 内部
- 部署手册更正为强制的后端先行顺序。
- 修复全部 6 个长期失败的测试;后端测试套自 5 月以来首次全绿(533 通过)。

## [0.21.0] - 2026-06-13

### 新增
- 跨语言精确引用:当你用一种语言提问、文档是另一种语言时(如中文提问、英文
  原文),或回答有改写时,引用现在会高亮原文中确切的支撑句,而非整段。回答生成
  后,由一个轻量模型逐源给出逐字支撑句并经验证后展示;若无法验证,则保留整段高亮。

## [0.20.0] - 2026-06-13

### 新增
- 句子级精确引用高亮:当回答的论断清晰对应到被引段落中的某一句时,在原文中只
  强调那一句,而不是点亮整块。保守策略——仅在某句明显胜出时收窄(带数字/日期
  一致性保护),否则保持整段高亮。适用于所有聊天回答。

## [0.19.0] - 2026-06-13

### 新增
- Quote Finder 验证基底（后端，尚未接入 UI）：偏移保持的文本规范化器 + 三层逐字
  引文验证器（"LLM 提议、验证器裁决、原文展示"），保证展示的引文一定是原文逐字。
  经 Codex 对抗审阅达成共识。

### 变更
- 聊天空状态重新展示按文档生成的建议问题（后端生成，无通用兜底）。
- 免费套餐上传上限由 25 MB 提升至 50 MB，让新用户在撞到任何付费墙之前先体验产品。
- 强化 finance / 10-K 分析在 finance 用例页的 SEO。

### 修复
- chunk 文本保真：句子切分不再破坏 `U.S.`、`e.g.`、小数或 `Why?Because`，重拼也不再
  在 CJK 句末标点后插入多余空格。
