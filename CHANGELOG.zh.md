# 更新日志

这里记录 DocTalk 产品的重要变更。

本项目遵循 [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)
和 [Semantic Versioning](https://semver.org/)。在 1.0 之前，DocTalk 使用
`0.minor.patch` 形式的版本号，例如 `0.2.0`、`0.2.1`。

## [Unreleased]

### 变更
- 德语和葡萄牙语首页的搜索标题改为与其他语种首页相同的信息（“与任何 PDF 即时对话”，
  用各自语言表达）。这两个页面自上线起一直显示大标题的主张，标题里没有“PDF”一词。
  日语标题去掉了一个多余的空格。其余页面的搜索标题和描述都不变。

### 修复
- 页面不再预加载 Fraunces 字体。改成夜间主题后，没有任何页面在加载时显示它。
  每个页面的关键路径因此少了 270 KB，在限速手机上最大内容绘制快了约 0.2 秒。
  唯一还用它的地方（聊天引用弹层里的引文）会在第一次用到时再加载。

## [0.32.0] - 2026-09-21

营销站点换成深色。所有营销页都改用「夜间」主题：无论设备是浅色还是深色设置，
都是暖调近黑的底，DocTalk 的赤陶、橄榄、琥珀三种角色色不变。App 界面保持原有
配色。计划文档：`.collab/plans/2026-09-21-landing-night.md`。

### 变更
- 首页一打开就是产品本身：Alphabet 2025 年第四季度财报第 1 页（逐字原文）在暗处，
  一束光找到被引用的那句话，荧光笔划过，然后出现带引用编号的回答。取代了之前
  静态的阅读器示意图。
- 所有营销页（定价、功能、使用场景、对比、替代方案、工具、演示、关于、联系、
  法律页面和博客）都使用夜间主题，标题改用 Geist 字体。
- 首页标题在 11 种语言中表达一致。
- 各语言营销页的搜索标题和描述改用独立的键，修改页面上的标题不会再连带改动
  搜索标题。本次发布后，每个页面的标题、描述和 Open Graph 文案都与之前一致。

### 修复
- 在常见笔记本屏幕尺寸下，11 种语言的被引用原句都保持完全点亮，回答卡片都在首屏内。
- 中文和日文的手机端标题不再夹带多余空格，日文标题按词断行。

### 开发者
- CI 会运行前端单元测试，包括首页文档文字与种子 PDF 逐字一致的检查。
- `frontend/scripts/design-audit/`：本次发布用来验证的截图、首屏几何、动画循环
  和对比度审计工具。

## [0.31.0] - 2026-09-21

营销站点改用苹果的构图语法——一个主张、一个操作，然后是产品本身——同时保留
DocTalk 的暖纸身份。计划文档：`.collab/plans/2026-09-20-apple-design-direction.md`。

### 变更
- 首页展示产品本身，而不是一张示意插画：阅读器回答一个关于真实文档（Alphabet
  2025 年第四季度财报）的问题，回答里的引用直接对应该页上被高亮的原句。标题、
  一句说明和一个操作排在最前；数字条、编号标签和标题的斜体拆分都已去掉。
- 定价页把方案放进首屏。手机上第一个价格原本在 2.31 屏之下、被一大段介绍挡住，
  现在首屏即可见。每个方案的价格紧跟在名称下方，只有推荐方案的按钮是实心的。
- 首页功能区精简为三项：引用、11 种语言、保留版式的翻译。
- 营销页跟随设备的浅色或深色设置，不再永远是浅色。深色是暖调近黑，而非简单反色。
- 标题在各尺寸下使用同一种展示字体，光学尺寸随实际字号变化，手机上词与词不再
  挤在一起。等宽标签和全大写眉标改为句首大写的普通文字。
- 玻璃效果仅用于页头和菜单；卡片、FAQ、横幅、表格改为实底。
- Cookie 同意横幅与营销页风格一致，手机上收成一行、不遮挡页面内容，按钮达到
  44pt 最小点击区域。页头的「登录」改为次级按钮，每页只有一个主要操作。

### 新增
- 首页在所有语言中都点名了引用查找器（Quote Finder），并使用各语言已有的产品名。
- 免费文档工具现在从站内可达：全站页脚「资源」栏新增「免费文档工具」入口，
  免费演示页和博客索引页各加一条上下文链接。此前 `/tools` 没有任何内部链接。

### 修复
- 所有设计颜色均达到 WCAG 4.5:1 的最低对比度，提升最大的是引用标记（3.23:1 →
  6.57:1）与小号标签（2.87:1 → 5.32:1）。
- 首页各区块不再在滚动到之前保持不可见。
- 阿拉伯语界面中，首页示例里的英文文档按从左到右排版。
- `/features/layout-translation` 与 `/trust` 的英文网址此前不在站点地图中
  （译文网址在，英文的缺失）。站点地图的英文条目改为从 `LOCALIZED_PATHS`
  派生，两份清单不会再各自漂移，并新增守卫测试。
- `/features/layout-translation` 的英文页此前不输出任何结构化数据，十个译文
  页只输出通用 Article。现在所有语言均输出 Article、BreadcrumbList 与
  SoftwareApplication，与同组功能页一致。
- 十个译文首页此前只输出 WebSite 与 Organization，而英文首页的 FAQPage、
  SoftwareApplication、HowTo 是硬编码英文。现在十一个首页都输出完整类型，
  并各自取用本语言的译文。
- 首页 HowTo 的步骤文案与页面实际渲染的内容不符。结构化数据与可见的
  「使用步骤」现在读取同一份来源，并有测试逐语言比对。

### 移除
- 一个未被使用的产品动画展示组件，及其两个依赖（`remotion`、`@remotion/player`）。

## [0.30.3] - 2026-09-13

### 修复
- 旧版订阅流程遗留 `pending` 标记时，系统会先与 Stripe 对账：复用仍有效的
  结账会话，继续拦截未结清订阅产生的重复购买，并在确认远端已无进行中状态后
  自动清除孤儿标记、创建新的幂等结账会话。

## [0.30.2] - 2026-09-13

### 变更
- Flash 聊天、匿名演示、文档工作流和版式翻译改用 DeepSeek V4.1 Flash 的
  规范模型名 `deepseek-flash`；旧名称继续用于历史用量记录和回滚兼容。
- DeepSeek 直连调用统一使用同一 provider 合同，明确重试、关闭产品模式中的
  thinking、传递匿名化用户隔离标识、控制 JSON 输出并记录模型/缓存/延迟指标。

### 修复
- 聊天 Action Planner 不再把全部输出额度消耗在隐藏推理后静默降级；现在会
  明确关闭 thinking 并要求返回 JSON 对象。

## [0.30.1] - 2026-09-13

- 账单卡片展示年度实付金额；管理员赠送套餐改为创建新订阅，同套餐意图按钮不再承诺无效操作。

### 变更
- PDF 引用采用浅琥珀原文标记，提供明确的区域/页面降级、适合宽度和返回回答；
  长引用及切页不再把无关词语误标为精确证据。
- 年付订阅按持久化月度计划发放积分，与 Stripe 每年扣款分开；购买入口由服务端发布开关控制。

### 修复
- 连续取消及断连时保存已输出的回答，积分只结算一次；完成数据库和响应流清理后释放会话。
- 重新生成成功保存后才替换原答案，保留历史，并阻止冲突写入；分享与导出使用当前答案。
- 账单使用实际 Stripe 价格和已持久化权益判断付款结果；未付清的升级不会生效，
  年付乱序/重复事件、退款和月度发放重试均有明确处理。
- 系统验收中已验证的文档恢复、窄屏控件、引用跳转、翻译脚注等问题。

## [0.30.0] - 2026-09-07

### 新增
- 打开文档时会直接看到文档讲了什么——一段摘要、要点和建议提问,不再是一个空对话框。
  这些内容此前一直在生成,只是自 5 月起就没有任何界面把它们显示出来。
- 处理失败的文档现在有"重试"按钮,仪表盘和阅读页都有。
- 回答失败时有了"重试"按钮,不再是死路一条、还把你输入的问题弄丢。

### 变更
- 处理失败的文档不再占用套餐的文档额度。失败文档另有自己的上限,不会无限堆积。
- 处理失败时的提示会说明到底哪里出了问题——文件读不了、扫描件没有可识别文字、
  下载失败——而不是所有原因都提示"请检查网络"。
- 仪表盘上的升级提示对真正在使用产品的人重新出现,并且对 Plus 的描述改准确了。

### 修复
- 连续快速点击两次"重试"不会再发出两次请求或重复扣费。
- 回答途中断网现在会明确报错并允许重试,不再让页面卡在"正在思考"的状态。
- 重新解析文档不会再与同时进行的删除操作互相卡死。

## [0.29.0] - 2026-09-03

### 新增
- 免费账户现在可以试用一次 Domain Mode。这个法律/学术对话模式是用户点击
  "升级"最主要的原因,而在此之前没有人真正见过自己被要求付费的功能。

### 变更
- 升级按钮现在直接打开 Stripe。此前应用内所有升级入口都只是把你送到账单页,
  真正的订阅按钮还要自己往下翻着找;现在表达意愿的那一次点击就开始结账。
- Plus 上传上限提升到 100 MB(此前 50 MB,与免费版相同),Pro 提升到 200 MB
  (此前 100 MB)。文件大小是用户最常撞上的限制,而 Plus 此前并没有真的放宽它。
- 文档新增页数上限:免费 750 页、Plus 1,500 页、Pro 3,000 页。现有文档全部
  在限额之内。
- 从网址导入统一为 10 MB 上限,所有套餐一致。
- 演示页直接展示示例文档,不再被标题、副标题和三步说明挡在下面。

### 修复
- 结账失败现在会告诉你到底哪里出了问题,而不是一句笼统的错误。
- "试试 Quote Finder"提示现在会在你用日常说法提问时出现——"在哪页写的"、
  "出处是什么"、"原文在哪"——而不再只认少见的学术措辞。它依然不会自行发起检索。
- 加密的 PDF 与无法确定长度的文件现在在上传时就会被明确拒绝并说明原因,
  不再先存下来、之后再失败。

### 内部
- 2026-08-26 自上而下复盘的增长批次 A 与 C。共十轮对抗审查(A 七轮、C 三轮)
  至共识。过程中发现并修复了后台文档任务积分结算中四个既有缺陷,其中之一
  会因为对已结算的工作重复退款而凭空产生积分。记录见
  `.collab/reviews/2026-08-27-batch-{a,c}-CONSENSUS.md`。

## [0.28.1] - 2026-08-08

### 修复
- 文档解析恢复正常速度。自 5 月底起,后端与数据库被平台调度到了不同大洲,
  每次数据库往返都要付出跨洋延迟,而解析又是逐行写入数千行数据——上传要等
  数分钟,超大文档甚至直接失败。现已把后端迁回数据层同区,并将页面、结构
  元素与切块改为批量写入。
- 解析超时的文档现在会如实报告为超时(并自动重试),不再被误标为数据库
  故障后放弃。

## [0.28.0] - 2026-08-04

### 变更
- 营销站全新视觉:暖纸底上的苹果风磨砂玻璃 chrome,配色更新(深锈红=行动、
  普鲁士蓝=结构、橄榄绿专属已验证状态),字体升级(Fraunces 标题衬线 +
  IBM Plex Sans)。正文保持实心表面——玻璃只用于导航、卡片与控件;所有玻璃
  表面在"减少透明度"与打印场景下回退为实心。
- 公开分享页并入营销设计体系,引文文本达到 AA 对比度。

### 修复
- 营销页头恢复吸顶(自 5 月起失效)——内容从磨砂玻璃下滚过。
- 暖纸背景现在真正渲染在每个营销页上;此前除落地页外均被静默压制。
- 分享的对话恢复段落换行。

### 内部
- Liquid Glass 计划第 1 批(营销/editorial 层)。16 提交,逐任务对抗审查;
  上线前发现并修复 7 个缺陷,含两个潜伏的生产 bug。记录见
  `.superpowers/sdd/2026-08-04-liquid-glass-*` 与
  `.collab/reviews/2026-08-04-glass-b1-*`。app 层为第 2 批。

## [0.27.0] - 2026-08-04

### 修复
- Quote Finder 现在会交出那些它本来找到、却被丢弃的引文。当一段话跨越翻页时,
  DocTalk 会以诚实的双页出处("pp. 30–31")展示它,而不是直接丢掉。用我们自己
  用户文档上的真实搜索重放,验证通过的引文从 10 条里的 1 条升到 8 条——其中
  包含此前被丢失的、一字不差的精确匹配。
- 复制跨页引文时,引注会正确写成 "pp. 30–31",不再悄悄只标首页。

### 内部
- 恰好跨一个页边界的 `extracted_text` 片段发出诚实区间并附两页 bbox;跨两个及
  以上边界仍按 `ambiguous_page_range` 丢弃;bbox 多数票猜页依然禁止。这是对 M2
  归因策略的一次有据修正案,依据是留存学术语料的生产内重放
  (`.collab/reviews/2026-08-04-m3-acceptance-gate.md`),并通过三轮 Codex 对抗审查。

## [0.26.0] - 2026-08-04

### 修复
- 法律模式与学术模式现已正确限定为 Plus 和 Pro 套餐。此前它们在界面上标为
  付费功能,但服务端对任何人都放行,免费账号直接请求即可获得付费行为。现在
  接受该参数的两处入口(聊天与结构化提取)都会校验套餐。
- 触达套餐上限不再是死胡同。分享链接用尽、版式翻译时文档额度已满、请求了
  未订阅的领域模式,现在都会给出明确的下一步,而不是一条干巴巴的报错。
- 已经是 Pro 却触达文档上限时,DocTalk 会提示你清理空间,而不再给出
  "降级到 Plus"的链接。

### 内部
- domain_mode 在两处请求入口均以 403 DOMAIN_MODE_REQUIRES_PLUS 门控;会话
  持久值在每条成功路径上同步为当前请求的值,且并入该路径自身的事务(无独立
  提交、无半提交元数据、无未退款窗口)。经两轮 Codex 对抗审查与真实 Postgres
  失败注入覆盖;记录见 `.collab/reviews/2026-08-0{3,4}-p1-*`。

## [0.25.0] - 2026-08-03

### 新增
- **证据看板 Evidence Board**:任意已验证引文卡一键保存,在 Quote Finder 的
  "已保存"标签页中随时找回——含可就地编辑的笔记、页码、信任标签、跳转高亮与
  带引注复制。个人资料页新增"已保存引文"标签,汇总你所有文档的保存引文。
  免费版含 30 条有效保存额度(删除即释放);重复保存同一条不重复计数。
- 保存时服务端重新验证——保存的卡片只可能包含经服务端对照你的文档验证过的
  文本,且每条都记录了验证依据以备将来复核。
- 账户数据导出现已包含你保存的引文与笔记。

### 内部
- 保存时验证锚(来源哈希+偏移);按用户的顾问锁封堵并发越限;为重解析引用
  动作补部分索引;保存引文全生命周期的真实 ASGI+Postgres 集成覆盖。四轮
  Codex 对抗审查(REVISE → r4 共识通过);记录见 `.collab/reviews/2026-08-03-m3-*`。

## [0.24.0] - 2026-08-02

### 新增
- **引文查找器 Quote Finder**(登录用户):在任意文档输入主题,获得经机器逐字
  验证的引文卡片——精确原文、页码、一键跳转到高亮段落、带引注复制
  ("作者, 年份, p. X")。未经服务端对照存储文本验证的引文绝不展示——结果会
  如实标注是对照原始页面文本还是提取文本验证的。
- 每文档可编辑的引注元数据(作者/年份/标题),自动从文件预填,始终可修改。
- 在聊天中请求"逐字引用/原文引用"会路由到验证式引文管线;含糊的表述则显示
  非侵入式"试试 Quote Finder"提示而不是猜测。引文搜索按 Pro 聊天消息计费;
  未验证出结果的搜索只收实际成本。
- 新上传的 PDF 现在按页存储原始文本,支撑页面级验证与页码精确归因。

### 修复
- Demo 示例文档启动时自愈其存储的 PDF 文件——此前一次存储迁移悄悄丢失了
  demo PDF(聊天正常但 PDF 面板打不开);已恢复,且此类问题不会再无声复发。
- 积分对账引入结算标记且并发安全:无论聊天还是引文搜索路径,扣费不会再被
  双重退款、结算后退款,或因请求取消而悬空。

### 内部
- 68 个提交,三波子代理团队开发,内部逐波审查 + 六轮 Codex 对抗审查
  (r1 BLOCK → r6 共识通过);记录见 `.collab/reviews/2026-08-02-quote-finder-m2-*`。
  集成测试改用隔离的临时数据库,仅允许本机回环提供。

## [0.23.0] - 2026-08-02

### 修复
- Demo 现在真正兑现宣传的额度:每个示例文档 5 条免费消息(此前是三个示例文档
  共享一个计数器),且计数在刷新页面、切换语言、网络波动后都保持准确——会自动
  与服务端计数重新同步。
- 在示例文档之间来回浏览不再悄悄消耗会话配额——回到某个示例文档时会恢复之前
  的对话而不是重新开始,正常浏览也不会再触发"请稍候"的限流提示。
- 示例文档不会再永久失效:会话上限改为 24 小时滚动窗口(此前是终身累计),每晚
  自动清理被遗弃的空会话,已登录账号也无法再耗尽匿名 Demo 的容量。
- 恢复了浅色模式下约 40 处因早前主题改造而"隐形"的界面元素:输入中动画、流式
  光标、输入框提示文字、Beta 徽章、点赞高亮、各类悬停状态等。深色模式不变。
- Demo 页现已提供全部 11 种语言的本地化网址(如 /de/demo),首屏即为翻译后的
  内容,对搜索引擎完全可见。
- 达到 Demo 上限后点击"上传你自己的文档",登录后会进入上传台而不是回到 Demo。
- Demo 中现在可以看到分享按钮,未注册访客会看到诚实的提示文案"登录后即可分享
  此对话"。

### 内部
- 匿名 Demo 消息计量按(IP,文档)维度;会话上限只统计 24 小时窗口内的匿名会话
  并配套部分索引;登录确认遥测(auth_confirm_*)现接受未登录客户端上报。
- 本批次由三波子代理团队开发,经 6 轮 Codex 对抗审查(r1 BLOCK → r6 共识通过)
  打磨;审查记录见 `.collab/reviews/2026-08-02-p0-*`。

## [0.22.0] - 2026-07-04

### 修复
- 邮箱登录链接现在会先打开确认页,点击按钮后才完成登录。此前企业邮箱的安全扫描
  程序会在收件人点击之前静默消耗一次性登录链接——造成死链和幽灵账号。如果你用
  工作邮箱登录时曾遇到 "Configuration" 错误,此问题已修复。
- 精确引用增强不再对匿名 demo 会话运行;增加了时间预算保护,长回答不会因此超过
  代理时限;运行时显示"Refining citations…"状态;其模型用量现已正确入账。
- 打开没有生成建议问题的文档时,不再显示上一个文档的建议问题。
- 更正了功能页、一篇博客与内部文档中过期的免费套餐文案(300 积分、50 MB 上传)。

### 内部
- 部署手册更正为强制的后端先行顺序。
- 修复全部 6 个长期失败的测试;后端测试套自 5 月以来首次全绿(533 通过)。

## [0.21.0] - 2026-06-13

### 新增
- 跨语言精确引用:当你用一种语言提问、文档是另一种语言时(如中文提问、英文
  原文),或回答有改写时,引用现在会高亮原文中确切的支撑句,而非整段。回答生成
  后,由一个轻量模型逐源给出逐字支撑句并经验证后展示;若无法验证,则保留整段高亮。

## [0.20.0] - 2026-06-13

### 新增
- 句子级精确引用高亮:当回答的论断清晰对应到被引段落中的某一句时,在原文中只
  强调那一句,而不是点亮整块。保守策略——仅在某句明显胜出时收窄(带数字/日期
  一致性保护),否则保持整段高亮。适用于所有聊天回答。

## [0.19.0] - 2026-06-13

### 新增
- Quote Finder 验证基底（后端，尚未接入 UI）：偏移保持的文本规范化器 + 三层逐字
  引文验证器（"LLM 提议、验证器裁决、原文展示"），保证展示的引文一定是原文逐字。
  经 Codex 对抗审阅达成共识。

### 变更
- 聊天空状态重新展示按文档生成的建议问题（后端生成，无通用兜底）。
- 免费套餐上传上限由 25 MB 提升至 50 MB，让新用户在撞到任何付费墙之前先体验产品。
- 强化 finance / 10-K 分析在 finance 用例页的 SEO。

### 修复
- chunk 文本保真：句子切分不再破坏 `U.S.`、`e.g.`、小数或 `Why?Because`，重拼也不再
  在 CJK 句末标点后插入多余空格。

### 文档
- 在 README、多语言 README、架构文档、RetainPDF 运维文档和 agent 交接指南中补齐
  保留排版 PDF 翻译说明。
- 新增当前 RetainPDF 产品流程、目标语言列表、预览与可选导入行为、套餐/页数/文件
  限制、部署检查清单，以及账单、发票、表单、表格密集型 PDF 的质量边界说明。

## [0.18.4] - 2026-05-24

RAG 检索、OCR 与回答质量整改（分 0.18.2–0.18.4 发布），由重放真实失败用户会话驱动。

### 修复
- 回答现在会在正文中标注准确的页码/幻灯片/工作表编号（而不仅在引用角标里），
  因此"请带页码引用""第 N 页有什么"等问题可正常回答。
- 助手不再拒答简短的关键词式提问（如一个地名或术语），而是检索文档后作答，
  或如实告知"未找到"。
- 助手不再向用户暴露内部术语（"片段""节选""chunk"）或其译法（如西班牙语
  "fragmento"），改为称"文档""正文"或具体页码。
- 扫描件及乱码的非拉丁 PDF（乌尔都语、阿拉伯语等）现在能被正确读取：内嵌字体
  提取为乱码的文档会被识别并用 OCR 重读，从而可被问答，而不再回复"文本是加密的"。

### 新增
- 追溯式、质量感知的 OCR 与自动脚本检测：解析器会标记"存在但乱码"的文本层，
  并根据文档实际脚本（而非 UI 语言）选用正确的语言重新识别。

### 变更
- OCR 改为使用按内容判定的窄语言集，非拉丁文档的提取文本明显更干净（且更快）。

## [0.17.1] - 2026-05-09

### 变更
- 降低公开 Demo 的首次体验阻力：示例文档会自动发送预设问题，并移除文档阅读页的
  阻塞式新手引导遮罩。
- 将 analytics consent banner 从全宽底栏改为紧凑浮层，避免遮挡文档输入框和移动端
  app tab。
- 改进移动端转化路径：`/auth` 首屏优先展示登录表单，并为公开站点 Header 增加紧凑
  移动导航菜单。
- 首页 hero 的核心价值主张现在首屏立即可见，不再等待入场动画 hydration。

## [0.17.0] - 2026-05-09

### 新增
- 新增 canonical `document_elements`，把 heading、paragraph 和已检测表格作为
  一等文档结构保存，避免全文任务继续只依赖任意 chunk window。
- 新增解析阶段的文本 element 生成，以及表格扫描阶段的 table element 生成；table
  element 会记录 table id、provider metadata、confidence 和 layout run 关联。
- 新增 element-aware 上下文选择，用于全文摘要、结构化提取、语义文档对比和
  table-aware retrieval。
- 新增 element 生成、代表性 element 覆盖、extraction/diff element-first context、
  table element 持久化，以及缺少同页 chunk 时表格 evidence fallback 的回归测试。

### 变更
- Chunk RAG 现在更明确地负责普通局部问答和 citation anchor；全文、表格和 diff
  工作流会先使用 canonical document structure 来获得更稳定的覆盖。
- 表格检索不会再因为 parser 没有生成同页 chunk 就丢弃已扫描表格；系统会 fallback
  到同文档 chunk 作为 citation anchor，同时保留 evidence payload 中的表格页码。

## [0.16.0] - 2026-05-09

### 新增
- 新增 Azure AI Document Intelligence `prebuilt-layout` provider，用于
  cloud-first PDF layout 和表格提取；SDK 采用 lazy import，避免本地或生产未配置
  Azure 时影响服务启动。
- 新增 `document_layout_runs` 表，记录 provider 执行、页数/表格数、原始 layout
  payload 的存储 key，以及失败原因。
- 新增 Azure 表格 cell metadata 保存，包括单元格区域、表头行/列、合并单元格 span、
  provider metadata 和 layout run id。
- 新增 Azure 表格映射、Azure 失败 fallback、layout run 记录、跨页续表合并以及 CSV
  转义的回归测试。

### 变更
- 表格扫描 job 现在对原生 PDF 优先使用 Azure layout analysis；当 Azure 凭据、SDK、
  鉴权、超时或服务调用失败时自动 fallback 到 PyMuPDF。
- 表格 artifact 轮询现在会展示使用的 provider，并在 Azure 不可用时显示 fallback
  warning。

## [0.15.0] - 2026-05-09

### 新增
- 新增 chat-native Action Planner，在每次聊天开始时判断用户是在普通问答、总结、
  生成 executive summary、提取关键事实、扫描/导出表格、创建模板、运行模板、做文档
  对比，还是定位引用页码。
- 新增聊天内工具 artifact。后台工具现在可以通过 SSE 推送 `tool_status` 和
  `artifact` 事件，把 artifact metadata 持久化到 `messages.metadata_json`，并通过
  统一的 `/api/document-jobs/{job_id}` 查询状态。
- 新增聊天 artifact card，支持 queued/running/succeeded/failed 状态、结果预览、
  引用按钮、Plus 限制的 CSV 下载入口，以及轮询刷新 job 状态。
- 新增整篇文档所有已检测表格的 Plus+ CSV 合并导出接口。

### 变更
- 文档阅读页现在只保留 Chat 作为主入口。Brief、Extract、Tables、Templates、Diff
  继续作为内部工具能力存在，但主页面不再展示独立的 `Brief` 或 `Extract` 标签。
- 推荐问题 chip 现在可以直接触发聊天内动作，例如“提取表格”和“对比版本”，不再把
  用户引导到新的工作区。

## [0.14.0] - 2026-05-07

### 新增
- 新增聊天回答的 claim-level RAG verification。DocTalk 现在会检查可由文档支持的
  回答是否带引用、引用编号是否对应本轮检索 evidence，以及引用文本是否与回答中的
  具体 claim 有实际重叠。
- 新增内部 `rag_verification_completed` 产品事件，用于持续监控回答质量，同时不通过
  公共 events API 暴露 bbox/chunk 等内部证据细节。
- 追加生成的 continuation response 也会写入同一套 verification reporting，避免长回答
  在质量面板中漏数。
- Admin 后台新增 RAG Quality 面板，展示已评估回答数量、平均验证分、pass/warn/fail
  比例、未引用 claim 数、低重叠引用数、数字不匹配数以及最近验证记录。
- 新增有效引用、缺失引用、无效 ref、未引用 claim、低重叠引用、admin 聚合、
  chat-stream verification event 写入和 continuation verification 的回归测试。

### 变更
- 引用重叠评分现在会先过滤常见英文停用词，避免因为 “the”“and” 等泛词重叠而把
  低相关引用误判为支持了回答中的 claim。
- 数字类 claim 现在要求引用来源上下文包含相同数字 token，避免表格值互相矛盾时
  仅因为实体名称重叠就被误判为通过。

## [0.13.0] - 2026-05-07

### 新增
- 新增确定性的 Query Planner，用于对比、多跳、穷尽扫描和多实体指标问题。计划化
  检索会补充受控子查询 evidence，不再只依赖一次普通 top-k 检索。
- 新增 Collection 对比问题的按文档均衡 evidence 覆盖，避免一个强匹配文档挤掉
  其他需要被比较的文档。
- 新增 query-plan prompt contract，指导模型综合多跳/对比 evidence，同时不会把原始
  planned query 回写进 system prompt。
- 新增 direct vs planned routing、实体/指标拆解、Collection 对比覆盖、计划化纠错
  检索以及 query-plan prompt 注入安全的回归测试。

### 变更
- 表格和数字问题现在会始终尝试 table/lexical evidence 与向量检索并行补充，即使
  vector search 看似足够，也能保留金融类短表格行的召回。

## [0.12.0] - 2026-05-07

### 新增
- 新增面向表格和数字类聊天问题的 table-aware retrieval。当文档已有
  `document_tables` 扫描结果时，匹配表格行会被格式化为可引用 evidence，并优先
  合并到普通向量/lexical 片段之前。
- 新增表格检索专用提示，要求数字回答精确保留行标签、单位、期间和币种。
- 新增结构化表格 evidence 排序、通用表格请求、特定词过滤、表格优先纠错检索、
  金融指标路由和表格 prompt guidance 的回归测试。

### 变更
- 表格/数字问题现在使用更低的 lexical chunk 长度阈值，避免短表格行在回答生成前
  被过滤掉。
- Query Router 现在可识别更多金融指标问题，例如 margin、target price、market cap、
  EBITDA、cash flow、币种和百分比，同时不会把普通页码查询误判为表格问题。

## [0.11.0] - 2026-05-07

### 变更
- 改进简单双栏 PDF 页面的解析阅读顺序，使左栏正文会先于右栏正文进入 chunk，
  不再按同一行左右交错拼接。
- 改进 chunk 文本拼接：英文 block 边界会保留必要空格，同时继续保持中文相邻字符
  和标点的紧凑拼接。
- 改进 micro-chunk 过滤：短文档不会再被整体过滤成没有任何可检索 chunk。

### 新增
- 新增 parser integrity 回归测试，覆盖双栏阅读顺序、页面中部全宽 block、
  Title Case 正文保留、英文句子边界拼接、短文档和中文标点拼接。

## [0.10.0] - 2026-05-07

### 新增
- 新增聊天检索质量 evaluator，在生成回答前评估普通 RAG 结果，识别空证据、
  精确词覆盖不足、穷尽扫描覆盖不足、向量分数过低、已命中词和缺失词。
- 新增纠错检索路径，用于局部问答、表格/数字问题、引用定位、存在性检查和
  穷尽扫描。该路径保留初始向量结果，再对同一文档范围执行 lexical fallback，
  按 chunk 去重合并，并将证据质量提示注入 prompt。
- 新增单文档和 Collection 的 lexical fallback 检索，基于转义后的 `ILIKE`
  匹配 chunk text 与 section title，并对精确标识符、数字和中文 bigram 进行
  确定性打分。
- 新增 evaluator 决策、纠错检索合并、聊天 prompt 质量提示和摘要路由隔离的
  回归测试。

### 变更
- 普通非摘要聊天现在通过 corrective retrieval wrapper，而不是直接调用一次向量
  检索。全文摘要和集合摘要仍继续使用持久化/代表性的 brief context 路径。

## [0.9.0] - 2026-05-07

### 新增
- 新增解析后持久化的分层 Document Brief。Brief 会保存精简摘要、结构大纲、
  关键要点、事实/数据、建议问题、生成元数据以及代表性 chunk 覆盖范围。
- 新增 `document_briefs` 表和 Celery `default` 队列 brief worker，使 parse
  任务完成时不再在解析任务内部同步执行 LLM 摘要生成。
- 新增 `GET /api/documents/{document_id}/brief`，访问控制与文档接口一致，
  并从 chunk/page/bbox 元数据补全可点击引用。
- 文档阅读页新增与 Chat、Extract 平级的 `Brief` 工作区，展示带引用的
  大纲、要点、事实、建议问题和状态，并支持点击引用跳转原文高亮。
- 新增 brief normalization、legacy summary 镜像、失败隔离、持久化覆盖检索、
  parse-worker 分发、brief API 授权与引用补全等回归测试。

### 变更
- 兼容旧 UI 的 `documents.summary` 与 `documents.suggested_questions` 现在由
  结构化 brief payload 派生。
- 全文总结路由现在优先使用持久化 brief coverage；缺失时再回退到实时代表性
  chunk 选择。

## [0.8.0] - 2026-05-07

### 新增
- 新增面向文档聊天的多标签 Query Router。类似 “summarize this document” 和
  “请总结这篇文档的要点” 的全文总结请求，现在会进入代表性全文摘要上下文路径，
  不再走普通语义 top-8 检索，并用 golden tests 覆盖全部 11 种支持语言。
- 新增代表性摘要上下文选择：按文档顺序覆盖开头、中部、章节变化和结尾，同时跳过
  过短的侧栏/页脚碎片。
- 新增文档集合总结路由：从集合内各文档抽取代表性覆盖片段，而不是用语义 top-k
  命中的零散片段总结整个集合。
- 新增 RAG 路由回归测试，覆盖多语言总结提示、表格/数字问题、存在性检查、
  文档集合对比候选和聊天集成路径。
- 新增 RAG 工作台执行台账，用于持续推进 router、brief、纠错检索、解析、
  表格、planner 和 verifier 阶段。

### 变更
- 全文总结提示现在使用专门的 summary system prompt，避免模型因为上下文是代表性
  摘录就把 ready 文档误判为“不完整文档”。
- 总结类聊天请求现在会预扣更高的 credits 估算值，降低更大代表性上下文带来的
  流式结束后补扣风险。

## [0.7.3] - 2026-05-07

### Changed
- public mobile header 现在将 `Sign Up Free` 作为主入口，Demo 调整为次级路径，
  恢复清晰的注册入口。

### Added
- 新增匿名且带限流的注册漏斗事件，包括 landing CTA 点击、auth modal 打开、
  OAuth provider 点击、email magic-link 结果；不记录邮箱地址或客户端 IP。

## [0.7.2] - 2026-05-07

### Fixed
- 新增 MinIO 后端内部 endpoint 与浏览器可访问 endpoint 的分离配置，使生产上传
  可走 Railway private networking，同时保持文档 presigned URL 可在浏览器访问。
- 当对象存储无法接收文件时，上传和 URL 导入现在会返回结构化
  `STORAGE_UNAVAILABLE`，不再暴露内部 500。

## [0.7.1] - 2026-05-07

### Fixed
- 修复 queued extraction、question-template、document-diff job 返回响应时触发
  async SQLAlchemy lazy-load 的问题，避免用户启动文档工作台异步任务后出现生产
  `HTTP 500`。

## [0.7.0] - 2026-05-07

### Added
- 新增 Pro-only Document Diff 工作流，可对比两份 ready 且属于同一用户的文档。
  报告按新增、删除、修改分组，并提供旧版/新版引用 chip，可跳回对应源文档。
- 新增全局 `/document-diff` 工作台与 Collection 内 Compare 标签，用户可从 dashboard
  或集合工作区启动版本对比。
- 新增 `document_diff` document job 类型、FastAPI 运行/列表/获取/导出接口，
  以及支持 credits 预扣费和实际费用 reconcile 的 Celery worker 任务。

## [0.6.0] - 2026-05-07

### Added
- 文档工作台新增可复用 Question Templates。用户可以创建保存的检查清单，
  在 Plus+ 上对单篇文档运行，并将带引用的答案矩阵导出为 Markdown 或 CSV。
- Pro 支持 Collection 批量模板运行，可对集合中的每份 ready 文档应用同一组
  问题，并展示带引用的答案矩阵。
- 新增 `question_templates` 表、`batch_template` document job 类型、FastAPI
  模板/运行/导出接口，以及用于队列化模板执行和 credits 预扣费/reconcile 的
  Celery worker 任务。

## [0.5.0] - 2026-05-07

### Added
- AI 回复操作区新增单条回答深链分享。回答分享复用现有 session share token，
  并附加稳定 message anchor，让接收者直接定位到对应的带引用回答。
- 公共分享页新增安全 message anchor，并在 URL fragment 命中时高亮目标回答。

### Security
- 公共分享响应继续隐藏 bbox 坐标、chunk id、document id、confidence score 等
  私有引用字段。

## [0.4.0] - 2026-05-07

### Added
- 文档工作台新增 Table Extraction，支持 `Tables` 预览、按需扫描表格、
  PDF `find_tables()`、非 PDF/转换文档的 markdown table fallback，以及 Plus+
  CSV 导出。
- 新增 `document_tables` 表与 `table_scan` document job 类型，为表格提取工作流
  提供可复用基础。
- 新增 FastAPI 表格扫描/列表/导出接口与 Celery table worker 任务。

## [0.3.0] - 2026-05-07

### Added
- 文档阅读页新增 Structured Extraction 工作区，支持 `Chat / Extract` 切换、
  三个带引用的提取模板、异步任务状态、引用跳转以及 Markdown/CSV 导出。
- 新增 `document_jobs` 与 `extraction_results` 表，作为文档工作台后续功能的
  通用异步任务基础。
- 新增 FastAPI 提取接口与 Celery `default` 队列任务，支持 queued extraction
  jobs、预扣费与实际费用 reconcile。
- 新增文档工作台连续交付执行台账。
- 新增自助取消订阅反馈采集，并支持记录可选退款审核请求。
- Pricing 与 Billing 页面新增 7-day fair-use refund review 文案。

### Changed
- 版本元数据升级到 `0.3.0 beta`；版本测试改为根据 `version.json` 推导下一
  个 patch dry-run，不再硬编码旧版本。
- 当前文档已更新为生产聊天模式 DeepSeek V4 Flash/Pro，并标明 Stripe 生产环境已使用 live billing。

## [0.2.0] - 2026-03-15

### Added
- 在 `version.json` 中集中维护产品版本号。
- 为前端和后端暴露统一的运行时发布元数据。
- 新增版本升级脚本与一致性检查脚本，纳入发布流程。
