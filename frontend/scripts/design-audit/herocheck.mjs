...
// usage: node herocheck.mjs <base-url> <out-dir>   → writes <WxH>.png, <WxH>-off.png, geometry.json
...
const GEOMETRY = `(() => {
  ...
  const gap = Math.min(64, Math.max(40, Math.round(stage.clientHeight * 0.06)));
  ...
  return {
    ...
    claimBottom: Math.round(cr.bottom), poolBottom: Math.round(cr.bottom + 40), passageTop: Math.round(cr.bottom + gap),
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    await send('Page.navigate', { url: `${base}/?still` });
    await sleep(3000);
    const g = await send('Runtime.evaluate', { expression: GEOMETRY, returnByValue: true });
    out[vp] = g.result?.result?.value || g.result?.exceptionDetails;
    const a = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(`${outDir}/${vp}.png`, Buffer.from(a.result.data, 'base64'));
    await send('Runtime.evaluate', { expression: `document.head.insertAdjacentHTML('beforeend','<style>.dt-editorial .ed-night-claim::before{display:none!important}</style>')` });
    await sleep(400);
    const b = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(`${outDir}/${vp}-off.png`, Buffer.from(b.result.data, 'base64'));
    process.stdout.write(`${vp} ${JSON.stringify(out[vp])}\n`);
  }
