...
import type { ChatArtifact, Citation, DocumentHierarchicalBrief, Message } from '../../types';
...
interface ChatMessageRowProps {
  ...
  onCitationClick: (c: Citation, messageId?: string) => void;
  onCitationSave?: (c: Citation, messageId?: string) => void;
    ...
    <MessageErrorBoundary messageId={message.id}>
      <div data-message-id={message.id} tabIndex={-1}>
        <MessageBubble
          ...
          onCitationSave={onCitationSave ? handleCitationSave : undefined}
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 pl-0">
            {uniqueCitations.map((citation) => (
              <CitationCard
                ...
                active={activeCitation?.chunkId === citation.chunkId && activeCitation?.refIndex === citation.refIndex}
                onClick={() => handleCitation(citation)}
              />
            ))}
          </div>
        )}
...
interface ChatPanelProps {
  sessionId: string;
  onCitationClick: (c: Citation, messageId?: string) => void;
  /** Save quote from a citation popover; wired by the document reader only. */
  onCitationSave?: (c: Citation, messageId?: string) => void;
  onPreviewLayoutTranslation?: (url: string, artifact: ChatArtifact) => void;
  ...
  documentBrief?: DocumentHierarchicalBrief | null;
...
export default function ChatPanel({ sessionId, onCitationClick, onCitationSave, onPreviewLayoutTranslation, maxUserMessages, suggestedQuestions, documentBrief, briefPolling = false, initialQuestion, onOpenSettings, hasCustomInstructions, userPlan, autoSubmitInitialQuestion = false, supportsCustomInstructions = true, onTryQuoteFinder }: ChatPanelProps) {
          ...
          {showDocumentBriefEmptyState && (
            <div className="flex min-h-full flex-col items-center justify-center px-2 py-8">
              <div className="dt-empty-workbench rounded-[1.75rem] px-5 py-6 sm:px-7 sm:py-7">
                ...
                {briefSummary ? (
                  <p className="line-clamp-4 text-sm leading-6 text-[var(--workbench-ink)] sm:line-clamp-none">
                    {briefSummary}
                  </p>
                ) : briefPolling ? (
                  <div className="space-y-2" aria-hidden="true">
                    <div className="h-3 w-full animate-pulse rounded bg-zinc-200 motion-reduce:animate-none dark:bg-zinc-700" />
