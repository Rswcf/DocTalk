...
export default function ModeSelector() {
  ...
  const choose = (modeId: ModeId) => {
    ...
    if (!isModeAvailable(modeId, userPlan)) {
      const reason = `${modeId}_mode`;
      if (!userPlan) {
        router.push(billingHref({ plan: 'plus', source: 'mode_selector', reason }));
        return;
      }
  ...
  return (
    <div>
      ...
      {checkoutError && <p role="alert" className="mt-2 text-xs text-red-600 dark:text-red-400">{checkoutError}</p>}
    </div>
  );
}
