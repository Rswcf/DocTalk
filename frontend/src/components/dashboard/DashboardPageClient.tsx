 ...
 * list, upgrade nudges, workspace nudge. Uses the functional app palette
 * (`dt-stitch-theme`, zinc+blue) — see `.claude/rules/frontend.md`.
 *
 * Extracted from `HomePageClient.tsx` (Wave-2 Q27). The Wave-1 fixes
 * preserved here:
 *   - C3: silent-delete is now try/catch with explicit error UI per row,
 *     and UI mutation only happens after the backend acknowledges.
 *   - C5: upload-parse polling timer is held in `pollTimerRef` and the
 *     useEffect cleanup at the top of this component clears it on unmount,
 *     so navigating away mid-parse doesn't leak a 2s interval forever.
 *   - Drag-leave already calls `setDragging(false)` on `onDragLeave`.
 */
