  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-8 gap-10">
      <div className="max-w-2xl w-full">
        <h1 className="text-3xl font-semibold text-center">DocTalk</h1>
        <p className="text-gray-600 mt-2 text-center">Upload a PDF to start chatting.</p>

        <div
          className={`mt-6 border-2 border-dashed rounded-xl p-10 text-center ${isDragging ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}`}
          onDragOver={(e) => { e.preventDefault(); setDragging(true); }}
          onDragLeave={() => setDragging(false)}
          onDrop={onDrop}
        >
          <input ref={inputRef} type="file" accept="application/pdf" className="hidden" onChange={onInputChange} />
          <p className="text-gray-700">Drag & drop your PDF here</p>
          ...
          {progressText && (
            <div className={`mt-4 text-sm ${uploading ? 'text-gray-600' : 'text-red-600'}`}>{progressText}</div>
          )}
        </div>
      </div>

      <div className="max-w-2xl w-full">
        <h2 className="text-lg font-medium mb-3">我的文档</h2>
        {myDocs.length === 0 ? (
          <p className="text-gray-500 text-sm">暂无历史文档</p>) : (
          <ul className="divide-y rounded-md border">
            {myDocs.map((d) => (
              <li key={d.document_id} className="p-3 hover:bg-gray-50 flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium">{d.filename || d.document_id}</div>
