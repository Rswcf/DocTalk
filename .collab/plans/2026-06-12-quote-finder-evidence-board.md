...
- **Page derivation from the verified slice, not the LLM emission.** For `page_start≠page_end` chunks there is no offset→page map (tables.py:140-145): page-scoped verification where `pages.content` exists; chunk-fallback must reject or split ambiguous multi-page matches. **[AMENDED 2026-08-04 by the M3 acceptance gate]** — "reject" is narrowed to segments spanning ≥2 page boundaries. A segment spanning exactly ONE boundary now emits an honest range (`pp. N–N+1`, bboxes from both pages), because production measurement showed the blanket discard was the dominant failure mode: 56% of prod PDF chunks span pages (89% of those by one boundary), `pages.content` exists for 11 of 108 documents, and four `exact 100.0` verbatim matches were being thrown away — the real-corpus replay scored 1/10 verified before and 8/10 after. Majority-bbox voting remains forbidden. Evidence: `.collab/reviews/2026-08-04-m3-acceptance-gate.md`.
