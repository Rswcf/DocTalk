...
**Date:** 2026-09-20
**Status:** Phase 0 and Phase 1 **shipped to production as v0.31.0** (2026-09-21T08:17:55Z, tag `v0.31.0` -> `bc39484`). Phase 2a (metadata decoupling) in progress; Phases 2b-5 not started. **The landing hero is superseded (2026-09-21):** the owner chose prototype B, Night — see `.collab/plans/2026-09-21-landing-night.md`. The `/` and `/[locale]` landing is `.dt-editorial.dt-night` (the editorial dark theme in both OS themes) with a canvas "citation field" hero; `ProductFrame` is deleted. The same day the owner extended Night to **every marketing page** (Geist display type, stage `#0a0908`) and paired its release with Phase 2a. Decision A2 (the app onto paper) was then **retired by the owner (2026-09-21): the app keeps its zinc/blue palette**; Phases 3–4 no longer re-point the app's colours.
...
#### Phase 1 execution notes — deviations from this plan, and why they were right
Recorded 2026-09-21 after the token layer landed. Each of these is a place where the plan as written would have produced a bug; the deviation is the correct behaviour and supersedes the plan text.

1. **Token names are `--ed-*`, not the bare `--stage`/`--surface`/`--inset` this plan proposed.** Two reasons, the second decisive: `--surface-2` already exists in `globals.css :root`, and — more importantly — roughly 40 components write `var(--ed-paper-2)` inline, and the untouchable `!important` counter-rule resolves `var(--ed-paper)`. A dark block under *new* names would never have reached any of them, leaving those components light-on-dark. **Mapping for later phases:** stage→`--ed-paper`, inset→`--ed-paper-2`, surface→`--ed-surface`, surface-2→`--ed-surface-2`, ink*→`--ed-ink*`, signal→`--ed-signal`, hairline→`--ed-rule`, plus `--ed-on-signal`, `--ed-r-1..4`/`--ed-r-pill`, `--ed-shadow-card`, `--ed-dur-1..3`/`--ed-ease`.
2. **`color-scheme: light dark` was NOT used** — it would have been a bug. next-themes runs with `enableColorScheme`, writing the concrete scheme inline on `<html>`; `light dark` on a subtree makes UA widgets follow the OS even when the user has explicitly forced the other theme. The explicit pair is used instead: `light` on `.dt-editorial`, `dark` on `.dark .dt-editorial`.
3. **Deleting the `.ed-display em` rule is not sufficient to retire the italic split.** The UA default for `<em>` is italic and Fraunces ships an italic face, so deletion alone leaves Latin headlines italic while `font-synthesis-style: none` renders zh/ja/ko/ar/hi upright — the reverse of the intent. `font-style: normal` on `em` within the three display steps is required.
4. **There are four `translateY(-1px)` hover rules, not five, and all four live in `globals.css` `.dt-*`** — inside the scope Phase 1 is forbidden to touch. Phase 1 item 8 and the Phase 1 exclusion list contradicted each other. They belong to **Phase 3**.
5. **`contrast_tokens.py` was itself broken** in a way that would have concealed exactly the failures it exists to catch: its end marker (`"html:lang"`) does not occur in `editorial.css`, so `str.find` returned `-1` and the slice ran to end-of-file. Harmless while one token block existed — with the dark block added, later declarations would have overwritten earlier ones and the "light" dictionary would silently have held **dark** hexes. Now fixed to parse a single rule block, with dark-inherits-light merging, a 3:1 non-text tier, and script-relative paths. Phase 0's published numbers are unaffected (every token name occurred exactly once then, and the values were independently confirmed against live production CSS), but any *other* prior output of that script should be treated as unverified.
6. **The label sweep is only half-complete and cannot be finished in Phase 1.** The CSS is sentence-case, but three English strings are literally uppercase — `landing.badge`, `pricing.eyebrow`, `landing.faq.eyebrow` (×11 locales) — and `EditorialHeaderBase` re-applies `textTransform: uppercase` inline. Those are copy and belong to **Phase 2**; `/pricing` still renders "PRICING" twice until then.
7. **`theme-color` is root-level**, so the paper tint now reaches the app's browser chrome while the app is still white. Under the owner's **A2** decision the app becomes paper in Phase 3 anyway, so this is correct-but-early rather than wrong. Revisit only if Phase 3 slips.
...
It is also a fifth instance of the seam: the banner is built entirely from the cool app palette — `--workbench-border`, `--workbench-panel-solid`, `--workbench-ink`, `--workbench-muted`, `dt-stitch-primary` (the blue button) and `zinc-*` hovers — and it renders on all 46 editorial routes. *(Corrected 2026-09-21: an earlier version of this note called its `backdrop-blur-2xl` "content-layer glass by the §5.4 rule". That was wrong — §5.4 explicitly lists the cookie banner among the legitimate functional-layer consumers, since a consent prompt is a transient control, like an alert. Glass was therefore permitted. It was removed anyway, on a different ground: the banner sits over busy content — the pricing cards — and HIG's regular variant exists for exactly that legibility case, which an opaque raised surface satisfies more simply.)*
...
Findings that change Phase 2's scope, in priority order:
1. **Four pages address nobody at the 0.70 bar** — `teachers` (max 0.25 across all five personas — the worst page on the site by this measure), `consultants` (0.52), `healthcare` (0.55), `compliance` (0.57). These are exactly the four with **zero measured users** in production. Restyling them is waste. Phase 2 should **consolidate or retire** them rather than port them to the new hero, and that decision belongs to the owner (added as Open Question 8).
2. **Two measured cohorts have no page at all.** P2 (student needing whole-book coverage) is the **largest measured segment and the #1 churn cause** (~10–12 of 47 analysed users left over it) and tops out at 0.53. P6 (cross-lingual reader — asks in Chinese about an English document; real users wrote in 10 languages) tops out at 0.62 despite 11 shipped locales and a `/features/multilingual` page. Phase 2 should *add* for these two before restyling pages nobody reads.
3. **Quote Finder is named on no marketing page at all** — not the landing, not `/features`, not `/pricing`, not the sitemap. It is the feature built specifically for P1, the only multi-week-retained cohort DocTalk has ever had. On `/use-cases/students` (which scores 0.90 for P1) "Find Quotes and Page Numbers" is the **fifth of five** help items, behind three that serve other personas. **This is the cheapest high-value change in the whole plan** and it is a *content* fix, not a design one — it should be pulled forward into Phase 1's `/` work, where the product frame already shows a verbatim highlighted passage with a page-accurate citation. The frame and the feature name should land together.
4. **The clause-finder job is served by two of its four pages** — `lawyers` (0.83) and `real-estate` (0.79) land it; `hr-contracts` (0.63) and `compliance` (0.57) do not, though all four target the same job. Diff the document-type cards and outcome wording of the two that work and apply it to the two that don't, rather than rewriting all four from scratch.

Two UX observations from the same session, both already anticipated here:
- The logged-out marketing landing **flashes before hydrating into the dashboard** for a signed-in user — this is the `HomePageClient` `status === 'loading'` seam named in §2.2. Independent confirmation that it is user-visible, not theoretical. Fixed in Phase 3 with the shared header; if it proves cheap, fix it in Phase 1.
- The free-plan document cap is 3 and the owner's own account sits at 3/3, which is what surfaces the upgrade nudge — relevant when judging Phase 1's `/pricing` work against a real account.
...
>    **Superseded 2026-09-21** (Night landing, `.collab/plans/2026-09-21-landing-night.md`): `ProductFrame` is deleted from the landing. Phase 4's visual spec is now `git show v0.31.0:frontend/src/components/landing/ProductFrame.tsx`.
...
| 4 | `app/d/[documentId]/DocumentReaderPageClient.tsx`, `components/Chat/*`, `components/PdfViewer/{PdfToolbar,PdfViewer}.tsx`, `components/Quotes/*`, `components/{DocumentBrief,Extraction,LayoutTranslation}/*`, `app/globals.css` (reader block, `.dt-answer-card`, `.dt-composer`), ~~`components/landing/ProductFrame.tsx` (→ real components)~~ (deleted 2026-09-21; spec at tag v0.31.0) |
