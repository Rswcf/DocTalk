...
**Date:** 2026-09-20
**Status:** Phase 0 and Phase 1 **shipped to production as v0.31.0** (2026-09-21T08:17:55Z, tag `v0.31.0` -> `bc39484`). Phase 2a (metadata decoupling) in progress; Phases 2b-5 not started. **The landing hero is superseded (2026-09-21):** the owner chose prototype B, Night — see `.collab/plans/2026-09-21-landing-night.md`. The `/` and `/[locale]` landing is `.dt-editorial.dt-night` (the editorial dark theme in both OS themes) with a canvas "citation field" hero; `ProductFrame` is deleted.
