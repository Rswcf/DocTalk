...
Production-wide: **8443 of 14919 PDF chunks (56%) span page boundaries**, and **7551 of those (89%) span exactly one** (page N→N+1). Because `extracted_text` has no offset→page map, the M2 Codex-r2 policy discards every one of them. The replay lost four `exact 100.0` verbatim matches this way — perfect quotes, verified, thrown away. For a span of one boundary, an honest "pp. N–N+1" is a serviceable academic citation; discarding is strictly worse for the user than reporting the range. **Correction (Codex, 2026-08-04):** an earlier draft of this report claimed plan §8.1 already sanctioned range labelling. It does not — §8.1 says chunk-fallback "must reject or split" ambiguous multi-page matches. Emitting a one-boundary range is therefore a deliberate **amendment** to the consensus, justified by the production data in this section, not a permission that already existed. The amendment is recorded in the plan, `.claude/rules/backend.md`, and both ARCHITECTURE docs so future agents do not undo it.
...
**This is a deliberate, production-data-justified policy reversal of one specific outcome inside the M2 Codex-r2 finding — not a regression.** The underlying r2 concern (majority-vote bbox guessing at a single page is unreliable) is still fully honored: the new code path never guesses a page, it reports a range. What changed is the response to irreducible ambiguity — discard vs. honest-range disclosure — and only for spans of exactly one page boundary. **This is an amendment to the consensus, not a permission it already granted**: plan §8.1 says chunk-fallback "must reject or split" ambiguous multi-page matches. The amendment is recorded 2026-08-04 in the plan itself, `.claude/rules/backend.md`, and both ARCHITECTURE docs, justified by the production measurement above. `test_quote_search_service.py`'s `TestAmbiguousMultiPageExtractedSegmentDiscarded` was renamed to `TestMultiPageExtractedSegmentAttribution` (its old name no longer matches its behavior — it now asserts emit-with-range for span 1, not blanket discard) and rewritten with this framing explicit in its class/test docstrings: one test asserts the span-1 emit case using the EXACT Codex r2 probe fixture, a new test asserts the span-2 discard case still holds. `TestVerifySavedQuote`'s equivalent test was updated the same way.

**Codex M3-r1 REVISE, both items addressed 2026-08-04:** (1) this rename, done as above. (2) LOW — the bbox range-filter test only had bboxes inside the reported 1-2 range, so deleting the `page_start <= bbox.page <= page_end` predicate while keeping every bbox would still have passed. Added a page-3 and a page-99 bbox to the span-1 fixture and asserted both are excluded from the emitted card's bboxes.

Mutation-tested three things, each broken then restored via exact string-replace: the span threshold (`>` vs `>=`), the bbox-range-filter reverted to majority-vote (reproduces the wrong page from Codex's own r2 probe), and the range predicate dropped entirely (leaks the out-of-range page-3/page-99 bboxes into the card) — all three caught by the test suite.
