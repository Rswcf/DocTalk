#!/usr/bin/env python3
"""Create or clean up an authenticated reader fixture for browser UX QA."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create/cleanup DocTalk browser reader QA fixture.")
...
async def create_user(plan: str) -> dict[str, Any]:
    ...
    email = f"qa-browser-reader-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}@example.com"
    async with AsyncSessionLocal() as db:
        user = await auth_service.create_user(db, email=email, name="QA Browser Reader")
        user.plan = plan
        await db.commit()
...
async def insert_messages(session_id: str, document_id: str, filename: str, chunk: dict[str, Any]) -> None:
    from app.models.database import AsyncSessionLocal
    ...
    async with AsyncSessionLocal() as db:
        ...
        sess.title = "QA browser citation/export session"
        db.add(sess)
        db.add(
            Message(
        ...
        db.add(
            Message(
                session_id=uuid.UUID(session_id),
                role="assistant",
                content="The document contains a semiconductor reading list and related notes. [1]",
                citations=[
                    {
                        "ref_index": 1,
...
def main() -> None:
    ...
    if args.command == "create":
        ...
        print(f"PASS: fixture created user={report['user']['id']} doc={report['document_id']} session={report['session_id']}")
    elif args.command == "cleanup":
        counts = asyncio.run(delete_user(uuid.UUID(args.user_id)))
        report = {"user_id": args.user_id, "cleanup_counts": counts, "result": "pass" if counts == {"users": 0, "documents": 0} else "fail"}
        write_report(args.json_out, report)
