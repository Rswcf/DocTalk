#!/usr/bin/env python3
"""Create or clean up an authenticated shared-session fixture for browser UX QA."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create/cleanup DocTalk browser sharing QA fixture.")
...
async def create_user(plan: str) -> dict[str, Any]:
    ...
    email = f"qa-browser-sharing-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}@example.com"
    async with AsyncSessionLocal() as db:
        user = await auth_service.create_user(db, email=email, name="QA Browser Sharing")
        user.plan = plan
        await db.commit()
...
async def create_fixture(args: argparse.Namespace) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(base_url=args.api_base, timeout=30.0) as client:
        share_res = await client.post(f"/api/sessions/{session_id}/share", headers=headers)
        share_body = safe_json(share_res)
        report["share"] = {"status_code": share_res.status_code, "body": share_body}
        share_res.raise_for_status()

    report["document_id"] = document_id
    report["session_id"] = session_id
    report["chunk_id"] = str(chunk["chunk_id"])
    report["assistant_message_id"] = inserted["assistant_message_id"]
    report["assistant_anchor"] = inserted["assistant_anchor"]
    report["share_token"] = share_body["share_token"]
    report["share_url"] = share_body["url"]
    report["result"] = "pass"
    return report
...
async def insert_messages(session_id: str, document_id: str, filename: str, chunk: dict[str, Any]) -> dict[str, str]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        sess.title = "QA shared browser session"
        db.add(sess)
        user_msg = Message(
            ...
            citations=[
                {
                    ...
                    "text_snippet": str(chunk.get("text") or "semiconductor reports")[:400],
                    "bboxes": chunk.get("bboxes") or [{"x": 0.1, "y": 0.2, "w": 0.3, "h": 0.1}],
        ...
        anchor = message_share_anchor(assistant_msg.id)
        await db.commit()
        return {"assistant_message_id": assistant_id, "assistant_anchor": anchor}
...
def main() -> None:
    ...
    if args.command == "create":
        ...
        print(f"PASS: sharing fixture created user={report['user']['id']} share={report['share_token']}")
        return
