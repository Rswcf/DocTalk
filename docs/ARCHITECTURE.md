...
**Chat features:**
- **Domain Mode trial ownership**: Plus/Pro access is unrestricted. Free accounts receive `FREE_DOMAIN_MODE_TRIALS` durable slots shared by chat sessions and extraction attempts. A slot-owning session can use Domain Mode for unlimited follow-ups. `feature_trial_usages` is the entitlement source of truth; owner FKs use `ON DELETE SET NULL`, so clearing a session's current mode or deleting its session/document/job never restores a consumed slot. Committed reservations survive downstream failures; only pre-commit validation/credit failures roll back a provisional extraction claim.
