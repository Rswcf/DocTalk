...
- **LLM Prompt and provider contract**: System prompts require `[n]` citations that match numbered document fragments. Production chat uses the official DeepSeek V4.1 endpoint: internal `quick` maps to canonical `deepseek-flash`, while `balanced` maps to `deepseek-v4-pro`; the retired `deepseek-v4-flash` identifier remains accepted for historical records. DocTalk explicitly disables thinking for both product modes, enables JSON mode for structured workflows, and sends only an HMAC-pseudonymized `user_id`. Provider telemetry records requested/actual model, latency, finish reason, usage, cache hits/misses, and failure type without prompt content. Anonymous demos use `DEMO_LLM_MODEL` (default `deepseek-flash`) to control cost. A **model-adaptive prompt system** (`model_profiles.py`) tailors rules and API parameters per model.
...
**Chat features:**
- **Domain Mode trial ownership**: Plus/Pro access is unrestricted. Free accounts receive `FREE_DOMAIN_MODE_TRIALS` durable slots shared by chat sessions and extraction attempts. A slot-owning session can use Domain Mode for unlimited follow-ups. `feature_trial_usages` is the entitlement source of truth; owner FKs use `ON DELETE SET NULL`, so clearing a session's current mode or deleting its session/document/job never restores a slot. Chat claims survive downstream failures because the same session remains retryable. A terminal failed/cancelled extraction releases its claim only when no result was delivered and its credit predebit is refunded in the same transaction; the user-row claim lock serializes release+claim, conditional deletion makes release idempotent, and successful extraction claims never release.
...
- `400` — `UNSUPPORTED_FORMAT` · `INVALID_FILE_CONTENT` · `PDF_PASSWORD_PROTECTED` · `FILE_TOO_LARGE {max_mb, plan}` · `URL_INVALID` · `URL_FETCH_BLOCKED` · `URL_CONTENT_TOO_LARGE` · `NO_TEXT_CONTENT` · `URL_FETCH_FAILED` · `INSTRUCTIONS_TOO_LONG {max}` · `CONTINUATION_LIMIT {max}` · `EXPORT_VALIDATION_FAILED {reason}`
