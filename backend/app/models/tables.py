...
class FeatureTrialUsage(Base):
    ...
    does not itself remove the consumed slot. An explicit terminal extraction
    failure may delete its undelivered job-owned row atomically with a credit
    refund. Historical NULL-owner extraction rows are released only when the
    orphan-ledger watchdog wins that same conditional refund; successful
    results and chat-owned rows remain durable.
    """
