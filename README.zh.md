...
**工作原理：** 文档被分割为 150-300 token 的文本块并附带边界框坐标，嵌入到 Qdrant 进行向量搜索，并先按问题类型路由。局部问题从 Qdrant 检索相关文本块；全文摘要请求使用覆盖文档开头、中部、章节变化和结尾的有序代表性上下文，避免长报告只被少数表格或侧栏片段带偏。引用映射回精确的页面位置，实现实时高亮。
