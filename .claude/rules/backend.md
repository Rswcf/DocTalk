...
- **Durable settlement (v0.24.0, extended to extraction workers 2026-08-26)**: `credit_ledger.reconciled_at` is the settlement marker. Every reconcile (async chat/Quote Finder and sync extraction-style workers) takes `SELECT ... FOR UPDATE` and ALWAYS stamps `reconciled_at` — including the equal-cost no-op path. Every refund is a single atomic conditional `DELETE ... WHERE reconciled_at IS NULL RETURNING id`; rowcount 0 = money already settled = no refund and no successful job/result overwrite. ALL final-commit exceptions (not just `CancelledError`) route through the marker resolver in a fresh session; resolver failure must NEVER fall through to a blind refund (leave predebit standing, log `*.unresolved`). When an extraction refund wins, its failed job state and eligible trial release commit in that same transaction. Do not reintroduce read-then-act refund logic.
...
## Parse Worker
- **Ingress cost caps**: upload and URL import count logical pages before object storage / `documents` INSERT. Limits are Free=750, Plus=1500, Pro=3000. Direct uploads retain the plan byte caps (50/100/200 MB). Every URL response — PDF or HTML, every plan, including after redirects — has one flat 10 MB cap enforced against both raw transport bytes and safely decoded output. URL fetching advertises only bounded gzip/deflate decoding and rejects Brotli/Zstd before decoding. Encrypted PDFs and files whose page count cannot be determined fail closed before storage / INSERT. Page-limit rejection is `DOCUMENT_PAGE_LIMIT_EXCEEDED` and must never consume a document slot.
- `time_limit=600`, `soft_time_limit=540`, `autoretry_for=(Exception,)`, max 2 retries, 60s backoff
- **Batched persistence only (v0.28.1, Codex 6-round consensus — see ARCHITECTURE §10)**: pages/elements/chunks go through `_insert_rows_batched` (executemany, 500/batch, one commit per stage); embedding backfills `vector_id` with one `UPDATE ... WHERE id IN (batch)`. Never reintroduce per-row ORM adds — a dense 10-page PDF has >1k element rows and wall time becomes rows × DB RTT (the 2026-08-08 incident).
- **Dispatch contract**: every dispatcher sets `status='parsing'` AND writes `parse_requested_locale` (value or intentional NULL) in the same commit BEFORE `.delay()` — via conditional UPDATE where racing is possible (reparse endpoint, backfill script). The worker serializes per document (advisory lock 947 + hashtext; unlock failure → `invalidate()`), no-ops on terminal status, only READS the stored locale (message locale argument is ignored).
- **Terminal-state gating**: only the FINAL autoretry attempt writes terminal status (`PARSE_TIMEOUT` incl. chained soft limits unwrapped from driver errors; `PARSE_FAILED` for generic), always via a fresh session; failed status writes re-raise for autoretry. Non-final attempts stay `parsing` (reparse 409s → no user-vs-autoretry race).
- **Stale watchdog**: beat task `requeue_stale_processing_documents` (30 min) claims docs with no writes >45 min (must exceed broker `visibility_timeout` 40 min) via conditional UPDATE. Startup recovery calls the same function — never blind re-dispatch of all in-flight docs.
...
- **Page attribution derives from the VERIFIED slice** (plan §8.1, **as amended 2026-08-04** — see below): an `extracted_text` segment spanning exactly ONE page boundary emits an honest range (`pp. N–N+1`, bboxes from both pages); a segment spanning ≥2 boundaries is still DISCARDED (`ambiguous_page_range`); `page_text` duplicates emit one card per matching page. **Never attribute via majority-bbox voting** — that remains forbidden; reporting a range is not the same as guessing a page inside it.
  - *Amendment record:* the original policy (M2 Codex r2) discarded EVERY multi-page `extracted_text` match, and plan §8.1 said chunk-fallback "must reject or split" them. The M3 acceptance gate (2026-08-04, `.collab/reviews/2026-08-04-m3-acceptance-gate.md`) measured the cost in production: 56% of prod PDF chunks span pages and 89% of those span exactly one, `pages.content` exists for only 11 of 108 docs, and the discard threw away four `exact 100.0` verbatim matches — the replay scored 1 verified out of 10 real user queries, rising to 8 once one-boundary ranges were emitted. The narrowed policy is therefore a deliberate, evidence-backed amendment, not drift. Do not "restore" blanket discarding.
