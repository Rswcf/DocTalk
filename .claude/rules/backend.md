...
## Parse Worker
- **Ingress cost caps**: upload and URL import count logical pages before object storage / `documents` INSERT. Limits are Free=750, Plus=1500, Pro=3000. URL responses are classified from the shared upload magic-byte validator, never `Content-Type`: byte-sniffed PDFs use the plan cap (50/100/200 MB), while every non-PDF response retains the independent 10 MB cap, including after redirects. Encrypted PDFs and files whose page count cannot be determined fail closed before storage / INSERT. Page-limit rejection is `DOCUMENT_PAGE_LIMIT_EXCEEDED` and must never consume a document slot.
