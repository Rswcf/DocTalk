...
- Free includes Flash plus a capped number of Pro answers/month. Export: Plus+ (frontend gated). Custom Instructions: Pro (backend gated). Sessions: Free=3/doc (backend gated). Saved quotes: Free=30 active across documents (backend gated; delete frees a slot). Quote searches cost credits like a Pro (balanced) chat message. Domain Mode (legal/academic chat overlay): Plus+ is unrestricted; Free currently receives a backend-enforced capped trial at both entry points (`chat.py`'s `chat_stream` and `extractions.py`'s `create_extraction`) and receives 403 `DOMAIN_MODE_REQUIRES_PLUS` after the current cap. The durable entitlement unit for that Free trial is pending an owner decision (Batch A finding 1); do not infer or document it as a session/request/message contract until decided.
