...
- Free includes Flash plus a capped number of Pro answers/month. Export: Plus+ (frontend gated). Custom Instructions: Pro (backend gated). Sessions: Free=3/doc (backend gated). Saved quotes: Free=30 active across documents (backend gated; delete frees a slot). Quote searches cost credits like a Pro (balanced) chat message. Domain Mode (legal/academic chat overlay): Plus+ is unrestricted; Free receives `FREE_DOMAIN_MODE_TRIALS` durable session slots shared with Domain Mode extraction attempts. A chat session that owns a slot may send unlimited Domain Mode follow-ups. Ownership lives only in `feature_trial_usages`, never `sessions.domain_mode`; deleting a session/document/job nulls the owner FK but does not restore the slot. Accepted downstream failures do not release reservations. Both backend entry points return 403 `DOMAIN_MODE_REQUIRES_PLUS` when no slot is available.
